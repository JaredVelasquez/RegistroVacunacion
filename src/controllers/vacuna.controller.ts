...
import {VacunaService} from '../services/vacunas.service';
import {Existencia} from '../temporalModels/vacuna';
...
export class VacunaController {
  constructor(
    ...
    public vacuna: VacunaService,
  ) {
    this.vacuna = new VacunaService(personaRepository, vacunaRepository);
  }
  ...
  async ExisteVacuna(
    @requestBody() existencia: Existencia
  ): Promise<any> {

    console.log(existencia.idPersona);
    let vacunasDisponibles = await this.vacuna.EnExistencia(existencia.idPersona);
